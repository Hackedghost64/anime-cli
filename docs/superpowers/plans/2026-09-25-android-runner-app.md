...
* "Dumb Runner" architecture: Extraction logic decoupled into `provider.bundle.js` evaluated in-memory by QuickJS with a headless WebView fallback.
* P2P Sync: `anime-cli sync` launches an ephemeral local HTTP server and outputs an ASCII QR code. The Android app scans the QR code and exchanges watch history deltas in <50ms over local Wi-Fi.
...
* **Script Engine**: QuickJS Android (JNI) + JavaScript ES2020.
...
### Task 1: The Provider Bundle & Test Suite (`provider.bundle.js`)

**Files:**
- Create: `android/app/src/main/assets/provider.bundle.js`
- Create: `tests/test_provider_bundle.mjs`
- Create: `package.json` (root or test runner config)

**Interfaces:**
- Produces: `provider.bundle.js` exporting:
...
- [ ] **Step 1: Write Node.js unit tests verifying provider contract**
...
- [ ] **Step 2: Implement `provider.bundle.js`**
  Implement Anilab API queries (`https://anilab2.amdapi.click/api`) and Kyoto Player stream extraction (`https://app.kyotoplayer.com/api/v4`) with AniSkip integration.
- [ ] **Step 3: Run provider test suite**
  Execute `node tests/test_provider_bundle.mjs` and confirm all tests pass.
...
### Task 2: P2P Local Sync Command (`anime-cli sync`)

**Files:**
- Modify: `cli.py` (add `sync` subcommand and `cmd_sync` handler)
- Create: `tests/test_sync_p2p.py`
- Modify: `db.py` (add delta query and batch upsert methods if needed)

**Interfaces:**
- Produces: `anime-cli sync [--port 8088]` CLI command.
- Exposes: `POST /api/sync` on ephemeral local server.
  - Header: `X-Sync-Token: <token>`
  - Request body: `{ progress_deltas: [...], script_version: string }`
  - Response: `{ ok: true, progress_deltas: [...], latest_script: string | null }`

- [ ] **Step 1: Write failing unit test for `anime-cli sync` endpoint**
  Create `tests/test_sync_p2p.py` using FastAPI `TestClient` to verify auth token validation and bidirectional watch progress merging.
- [ ] **Step 2: Implement delta sync and batch update in `db.py`**
  Add `get_progress_deltas(since_timestamp: int)` and `merge_progress_deltas(deltas: list)`.
- [ ] **Step 3: Implement `cmd_sync` and router in `cli.py`**
  Implement ephemeral server, token generation, QR code rendering, and auto-shutdown on sync complete.
- [ ] **Step 4: Run pytest on sync module**
  Execute `python3 -m pytest tests/test_sync_p2p.py` and verify all tests pass.
- [ ] **Step 5: Commit Task 2**
  `git add cli.py db.py tests/test_sync_p2p.py && git commit -m "feat(sync): add P2P anime-cli sync command with QR authentication"`

---

### Task 3: Android Project Scaffolding & QuickJS Engine Bridge
...
- Create: `android/app/src/main/java/com/shinsei/anime/runner/QuickJsBridge.kt`
...
- [ ] **Step 1: Create Gradle scaffolding for Android app**
  Configure root and app `build.gradle.kts` with AndroidX Media3, Compose, Material3, Room, and QuickJS dependencies.
- [ ] **Step 2: Implement `QuickJsBridge` & `ScriptRunner`**
  Load `provider.bundle.js` from internal storage (falling back to asset bundle) and wrap JS promises into Kotlin suspend functions.
- [ ] **Step 3: Verify build with Gradle**
  Run `./gradlew test` or `./gradlew compileDebugKotlin` in `android/`.
- [ ] **Step 4: Commit Task 3**
  `git add android/ && git commit -m "feat(android): scaffold project and implement QuickJS ScriptRunner bridge"`
...
- [ ] **Step 2: Implement `SyncClient`**
  Parse `shinsei://sync?host=...&port=...&token=...`, perform HTTP POST via `OkHttp`, merge deltas, and write updated script to disk if returned.
- [ ] **Step 3: Unit test Room DAO and SyncClient parsing**
  Write JVM local unit tests verifying QR payload extraction and delta merging logic.
...
- UI Features:
  ...
  - Double-Tap Seeking: Curved ripple animation with cumulative second indicator.
  - Dual-Axis Gestures: Vertical swipe on left for brightness, right for volume.
  - Android Picture-in-Picture (`PictureInPictureParams`).
...
- [ ] **Step 4: Connect PiP & Lifecycle hooks**
  Implement `onUserLeaveHint` for Picture-in-Picture and progress auto-save on exit.
...
- [ ] **Step 1: Implement OLED Cinema Theme**
  Define color tokens (`#08090D`, `#FF640A`, `#12151E`) and typography.
- [ ] **Step 2: Build Home Feed Screen**
  Implement Hero Billboard and horizontal LazyRow rails including "Continue Watching".
- [ ] **Step 3: Build Detail Screen**
  Implement banner backdrop, metadata strip, Sub/Dub toggle pill, and episode list.
- [ ] **Step 4: Build QR Scanner Sheet**
  Implement CameraX / ZXing barcode scanner bottom sheet connected to `SyncClient`.
